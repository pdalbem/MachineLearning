...
#define N 10                 // variáveis
#define CLAUSES 15           // número de cláusulas

#define POP_MAX 100
#define INIT_POP 15
#define GENERATIONS 50      // Aumentado para melhor convergência
#define ELITE_SIZE 5

#define MAX_PARENTS 2
#define THRESHOLD 10         // Aumentado para filtrar soluções ruins
#define SIM_THRESHOLD 3      // Aumentado para preservar diversidade
...
/* cada cláusula tem até 3 literais */
int formula[CLAUSES][3] = {
    {1, -2, 3}, {-1, 2, 4}, {2, 3, -4}, {-3, -4, 5},
    {1, 5, -6}, {-2, 6, 7}, {3, -7, 8}, {-5, -8, 9},
    {6, 9, -10}, {-1, -3, 10}, {2, -4, 5}, {-6, 7, 8},
    {3, 4, -9}, {-2, -8, 10}, {1, -5, 6}
};
...
Node bn[N];
double cpt[N][4][2];  // 4 configurações (2^2)

/* ================= FITNESS ================= */

int eval_clause(int clause[3], int x[]) {
    for (int i = 0; i < 3; i++) {
        int var = abs(clause[i]) - 1;
        int val = x[var];

        if (clause[i] > 0 && val == 1) return 1;
        if (clause[i] < 0 && val == 0) return 1;
    }
    return 0;
}
...
int fitness(Individual *ind) {
    ...
    for (int i = 0; i < CLAUSES; i++)
        sat += eval_clause(formula[i], ind->x);
    return sat;
}
...
int main() {
    ...
    for (int g = 0; g < GENERATIONS; g++) {
        ...
        // Amostragem - gera novos indivíduos a partir dos melhores
        int new_size = pop_size;
        int samples = POP_MAX - pop_size;
        
        // Seleciona indivíduos para amostragem (favorece os melhores)
        for (int i = 0; i < samples; i++) {
            // Seleção por torneio
            int idx1 = rand() % pop_size;
            int idx2 = rand() % pop_size;
            int base_idx = (pop[idx1].fitness > pop[idx2].fitness) ? idx1 : idx2;
            
            Individual new_ind;
            // Copia o indivíduo base
            for (int j = 0; j < N; j++)
                new_ind.x[j] = pop[base_idx].x[j];
        ...
        int k = 0;
        for (int i = 0; i < pop_size; i++) {
            // Mantém elites e indivíduos com fitness >= threshold
            if (pop[i].fitness >= THRESHOLD || i < elite_count) {
                pop[k++] = pop[i];
            }
        }
        pop_size = k;
    ...
    printf("\nSatisfied clauses = %d / %d\n", best_fitness_ever, CLAUSES);
    
    // Mostra quais cláusulas foram satisfeitas
    if (best_fitness_ever < CLAUSES) {
        printf("\nUnsatisfied clauses:\n");
        for (int i = 0; i < CLAUSES; i++) {
            if (!eval_clause(formula[i], best_solution.x)) {
                printf("  Clause %d: ", i+1);
                for (int j = 0; j < 3; j++) {
