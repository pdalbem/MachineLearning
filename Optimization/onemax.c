...
#define POP_MAX 300
#define INIT_POP 100
#define GENERATIONS 200    // Reduzido porque OneMax converge rápido
#define ELITE_SIZE 5

#define MAX_PARENTS 2
#define THRESHOLD 10       // Mantém diversidade mas filtra soluções muito ruins
#define SIM_THRESHOLD 5    // Aumentado para preservar diversidade
...
void learn_structure(Individual pop[], int size) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        while (bn[i].num_parents < MAX_PARENTS) {
            ...
            // Permite qualquer nó como pai (exceto i)
            for (int p = 0; p < N; p++) {
                if (p == i) continue;
...
int main() {
    ...
    for (int g = 0; g < GENERATIONS; g++) {
        ...
        // Aprende BN (usando toda a população, não metade)
        if (pop_size >= 10) {
            learn_structure(pop, pop_size);
            learn_parameters(pop, pop_size);
        }
        
        // Amostragem - gera novos indivíduos
